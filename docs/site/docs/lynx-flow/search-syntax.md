...
The search expression is the first part of a pipeline (before the first `|`). It determines which events enter the pipeline. In Lynx Flow, canonical queries start with `from <source>` followed by `| search` or `| where`. The search expression syntax below is used with `search` and in the pre-pipe position.
