...
| Rewrite transparency | `NormalizeQueryWithRewrites` records structured `Rewrite{Before, After, Reason}` entries for normalizer rewrites; REST query sync and accepted responses expose them as `meta.rewrites`; `lynxdb query --show-rewritten` prints rewrite metadata in server, TUI, file, and stdin modes | Completed job lookup responses do not yet replay stored rewrite metadata. |
...
| REST `meta.suggestions`, `meta.explain`, and persisted job rewrite replay | Deferred | Query submit responses now expose `meta.rewrites`; suggestions, explain blocks, and job-snapshot rewrite retention still require API contract expansion without changing result row shape. |
| Persisted job `--show-rewritten` replay | Deferred | Query submit responses include rewrite data and CLI/TUI can render it immediately, but `/query/jobs/{id}` snapshots do not retain rewrites for later replay. |
