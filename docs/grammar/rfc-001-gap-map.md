...
| REST `meta.explain` advisory blocks | Deferred | Query submit and job lookup responses now expose lint-derived shortcut suggestions; explain blocks still require planner metadata without changing result row shape. |
