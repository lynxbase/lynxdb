...
| Rewrite transparency | `NormalizeQueryWithRewrites` records structured `Rewrite{Before, After, Reason}` entries for normalizer rewrites; REST query submit and job lookup responses expose them as `meta.rewrites`; `lynxdb query --show-rewritten` prints rewrite metadata in server, TUI, file, and stdin modes | Explain output does not yet expose a dedicated rewrites block. |
...
| REST `meta.suggestions`, `meta.explain`, and explain rewrite blocks | Deferred | Query submit and job lookup responses now expose `meta.rewrites`; suggestions and explain blocks still require API contract expansion without changing result row shape. |
