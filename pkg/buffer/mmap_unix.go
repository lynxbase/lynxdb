...
// The returned pointer must be freed with freeOffHeap when no longer needed.
func allocateOffHeap(size int) (unsafe.Pointer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("buffer.allocateOffHeap: mmap(%d bytes): %w", size, err)
	}

	return unsafe.Pointer(&data[0]), nil
}

// freeOffHeap releases off-heap memory allocated by allocateOffHeap.
func freeOffHeap(ptr unsafe.Pointer, size int) error {
	...
	if err := unix.Munmap(data); err != nil {
		return fmt.Errorf("buffer.freeOffHeap: munmap(%d bytes): %w", size, err)
	}
